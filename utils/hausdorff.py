...
"""
def HDLoss(pred_: torch.Tensor, target_: torch.Tensor):
    """
    ...
    #bchw
    pred = (torch.split(pred_, 1, dim=1))[1]
    target = (torch.split(target_, 1, dim=3))[1].transpose(2, 3).transpose(1, 2)
    pred = torch.softmax(pred.transpose(1, 2).transpose(2, 3).contiguous().view(pred.shape[0], -1, 1),-1) #bhwc
    pred=torch.abs(torch.sub(pred,1.0))
    target=torch.abs(torch.sub(target,1.0))
    pred=pred.view(size=(3,512,512,1)).transpose(2,3).transpose(1,2)

    pred_dt = torch.from_numpy(distance_field(pred.cpu().detach().numpy())).float()
    target_dt = torch.from_numpy(distance_field(target.cpu().detach().numpy())).float()
    alpha=2.0
