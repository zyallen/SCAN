...
class Attention_SG(nn.Module):
    ...
    def forward(self, g, x):
        ...
        gx = self.conv(g1+x1)
        psi = self.relu(gx)
        # psi1 为 local_attention_matrix
        psi1 = self.psi(psi)
        # psi2 为 channel_attention_matrix
        psi2= self.spa(g1+x1)
        out1 = psi1 * (g1+x1)
        out2 = psi2 * (g1+x1)
...
class IIAM(nn.Module):
    ...
    def forward(self, g, x):
        ...
        # psi2 为 channel_attention_matrix
        psi2= self.spa(g1+x1)
        # Calculate the similarity of adjacent matrices
        ...
        for i in range(len(psi1)):
            ...
            for j in range(len(psi1)):
                ed.append(torch.norm(psi1[i]-psi1[j]))
                # ed.append((np.linalg.norm(psi1[i].cpu().detach().numpy() - psi1[j].cpu().detach().numpy())))
                ed_total = ed_total + ed[-1]
            for k in range(len(psi1)):
                ed_temp = 1.0 - (ed[k] / ed_total)
                temp = psi1[i].clone() + psi1[k].clone() *ed_temp
            psi3[i] = temp
        ...
        for i in range(len(psi2)):
            ...
            for j in range(len(psi2)):
                ed_c.append(torch.norm(psi2[i]-psi2[j]))
                # ed.append((np.linalg.norm(psi1[i].cpu().detach().numpy() - psi1[j].cpu().detach().numpy())))
                ed_total_c = ed_total_c + ed_c[-1]
            for k in range(len(psi2)):
                ed_temp_c = 1.0 - (ed_c[k] / ed_total_c)
                temp_c = psi2[i].clone() + psi2[k].clone() * ed_temp_c
            psi4[i] = temp_c
        ...
        out2 = psi4 * (g1+x1)
        out=self.sigmoid(out1+out2)
        # The final splicing operation is in unetUp_G Completed
        return g1*out,x1*out
